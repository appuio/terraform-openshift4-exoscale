...
variable "additional_worker_groups" {
  type    = map(object({ type = string, count = number, data_disk_size = optional(number), state = optional(string), affinity_group_ids = optional(list(string)), use_instancepool = optional(bool), region=optional(string), template_id=optional(string) }))
  default = {}
